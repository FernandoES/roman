...
public class RomanNumberConverter {
    ...
    public List<String> convertArabicToPseudoRoman(int arabic) {
        IntStream  streamOfChars = Integer.toString(arabic).chars();
        return streamOfChars.mapToObj(num -> (pseudoCodeRoman[Character.getNumericValue(num)])).collect(Collectors.toList());
    }

    public String convertPseudoRomanToRoman(List<String> pseudo) {
        return IntStream.range(0, pseudo.size()).mapToObj(index -> (converPseudoRomanSingleDigit(pseudo.get(index), pseudo.size() - index -1))).collect(Collectors.joining(""));
    }

    public String converPseudoRomanSingleDigit(String pseudoDigit, int position) {
